// Web Worker para processar dados sem bloquear a thread principal
...
self.onmessage = (event: MessageEvent<ProcessingMessage>) => {
  ...
  const { opportunities, actions } = event.data;
  ...
  try {
    ...
    const kpis = { totalOps: seenOps.size, totalAgendas };
